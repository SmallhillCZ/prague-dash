...
export function registerOpenAPI(path: string, app: INestApplication, config: Config) {
  ...
  let prefix = config.server.globalPrefix ? `/${config.server.globalPrefix}` : "";
  ...
  addOpenapiPaths(openapiDocument, path, config);
 ...
 */
function addOpenapiPaths(document: OpenAPIObject, path: string, config: Config) {
  const ROOT_TAG = "Root";
